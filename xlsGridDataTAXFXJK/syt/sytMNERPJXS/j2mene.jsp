 < %@page contentType = "text/html;charset=UTF8" % ><%@page import = "com.xlsgrid.net.pub.*,com.xlsgrid.net.mobile.MobileLogin,com.xlsgrid.net.web.*" % ><html > <head > <meta http - equiv = "Content-Type"content = "text/html; charset=UTF8" > <title > main < /title>
</head > <body >
...
if (bContinue) {
    try {
        ...
        if (org.equalsIgnoreCase("SHMNJXS")) {
            ...
            sql = "select distinct c.extcat2 from usrcorp a,usr b,v_ka c,corp d " + "where a.usr=b.guid and a.corp=d.guid " + "and c.guid=d.refguid and b.id='" + sUID + "' order by c.extcat2";
            ds = EADbTool.QuerySQL(sql);
            int row = ds.getRowCount();
            String extcat2 = "";
            boolean sign1 = true;
            ...
            String sql4 = "select ownloc from usr where id = '" + sUID + "'";
            ds = EADbTool.QuerySQL(sql4);
            if (ds.getRowCount() > 0) {
                String ownloc = ds.getStringAt(0, "ownloc");
                out.print("<img src=\"resource://pages/5.png\"> <a href=\"j2meplanmngdate.jsp\">条线经理查询</a><br>");
            }
            String sql1 = "select * from USRCORPBASCAT12 where usr=(select guid from usr where id='" + sUID + "')" + "union all select * from M_USRCORPBASCAT12 where usr=(select guid from usr where id='" + sUID + "')";
