...
//                      if ( nnn == 0 ) out.println( "update curord set qty="+sqlstr+" where guid='"+sGuid+"'" );
              ...
              if ( balrow > 0 )
              {
                    String balsql = " select round(sum( decode(curord.sctype,'1',nvl(corpitem.price,0)/item.untnum)*curord.qty),2) balmny from curord,corp,corpitem,item where curord.corp=corp.guid"+
                    ...
                    d_mny = Double.parseDouble(balds.getStringDef(0,"bal","0"));                  
                    if ( d_balmny > d_mny )
                    {
                        db.Rollback();
