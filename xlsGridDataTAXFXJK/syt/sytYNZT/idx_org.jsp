...
String a_idxid[]={"838","121","578","672","726","750","202","203"};
...
<!--
  ...
  {
    var sql="select '"+idxid+"' myidxid,orgid tip,orgnam 组织,"+
		...
		" from <%=dcfinUser%>v_baseindex where idxid='"+idxid+"' and idxitmid='"+idxitmid+"' and "+
    " year="+document.all("SEL_YEAR").value+" and orgid like '"+G_SELORG+"[PERCENT]' "+
    " AND not(money01=0 and money02=0 and  money03=0 and  money04=0 and  money05=0 and  money06=0 and  money07=0 and  money08=0 and  money09=0 and  money10=0 and  money11=0 and  money12=0 ) "+
    " group by orgid,orgnam,idxitmid,idxitmnam,ORGSORTID order by ORGSORTID"; //and orgid='"+G_SELORG+" ','0000512677','0000317815'

    f_showtable_ex(sql,"#",title+"按年度(累计数)",bShowSum,"BYYEAR",bAddSerail);
  ...
  function f_showtable_ex (sql,link,title,bShowSum,tagid,bAddSerail)
  {
    ...
    //alert ( namelist );
    newChart.SetData(c.chDimCategories, c.chDataLiteral, namelist);// 横坐标
    var sumrow = _this.XMLDS_AddRow(_this.XMLDS_GetRowCount()-1,"合计");
    _this.XMLDS_SetStringAt(sumrow,2,"合计(万元)");
    var oldsum = 0;
    for ( var col=3;col<colcnt;col++ ){
    	var sum = 0;
    	for(var i=0;i<rowcnt;i++ ) {
    		var s = _this.XMLDS_GetStringAt(i,col);
    		// 去掉
    		s = s.replace( "万","");
    		sum += 1.0*s;
    	}
    	_this.XMLDS_SetStringAt(sumrow,col,sum);
    	oldsum= sum;// 累计数->发生数
    }
	...
	for ( var col=3;col<colcnt;col++ ){
		if ( col != 3 ) valuelist+=",";
		var ss = _this.XMLDS_GetStringAt(sumrow,col);
		if ( ss.length == 0 ) ss = "0";
		valuelist+=ss;
	}   		
	var s = newChart.SeriesCollection(0);
    ...
    for ( var i=0;i<rowcnt;i++){
      ...
      str += "<tr height='25' bgcolor='"+TdbgColor+"' ";
      str += " onmouseover=\"this.style.backgroundColor='#D3E5FA'; this.style.cursor='hand'; title='"+_this.XMLDS_GetStringAt(i,1)+"'\" onmouseout=\"this.style.backgroundColor='"+TdbgColor+"';\" " ;
      str += " onclick=\"window.location='"+link + _this.XMLDS_GetStringAt(i,0)+"';\" >";
      for(var j=2;j<colcnt;j++)
      {
        //设置单元格对齐方式
        if ( j == 2 )
    ...
    var DataTab = TabH + str + TabE ;
    var strTemp = "";
    strTemp+="<table border=\"1\" cellspacing=\"0\" id=\"table5\" height=\"200\">";
    strTemp+="	<tr>";
    strTemp+="		<td style=\"border-left: 1px solid #3A77BA; border-right: 1px solid #3A77BA; border-bottom: 1px solid #3A77BA; border-top: 1px solid #3A77BA;\" bgcolor=\"#FFFFFF\"  valign=\"top\">";
    strTemp+="			<table  cellspacing=\"1\" ><tr><td align='center' colspan="+(colcnt-2)+">"+title+"</td></tr><tr><td><font color=\"#014E82\">"+str +"</font> </td></tr></table>";
    strTemp+="　	</td>";
    strTemp+="	</tr>";
    strTemp+="</table>";
    //alert ( strTemp );
                      ...
                      <table width="100%"><tr><td width=152>&nbsp;<img border=0 src=../xlsgrid/images/my/collapse.gif width=9 height=9> 
                        ...
                        <select size="1" name="SEL_ORG" onchange="javascript:G_SELORG=document.all('SEL_ORG').value;f_refresh();">
									<%=strIdxtypOrg%>
																		...
																		选择年 <select size="1" name="SEL_YEAR" onchange="javascript:f_refresh();">
...
　</td>
							...
							<table border="0"  id="table7" width="134%">
								<tr>
									<td valign="top"></div>
								...
								<tr>
									<td valign="top">
<object classid="clsid:0002E55D-0000-0000-C000-000000000046" id="Chart_BYYEAR" width="727" height="200">
...
<table width='100%' cellpadding='0' cellspacing='0' border='0' height='8'><tr><td bgColor='#336699' height='25' width='10%'>&nbsp;</td><td bgColor='#666666'width='85%'><font face='宋体' color='white' size='4'><b>&nbsp; 缺少 Microsoft Office Web Components</b></font></td></tr><tr><td bgColor='#cccccc' width='15'>&nbsp;</td><td bgColor='#cccccc' width='500px'><br> <font face='宋体' size='2'>此网页要求 Microsoft Office Web Components。<p align='center'> <a href='G:/files/owc11/setup.exe'>单击此处安装 Microsoft Office Web Components。</a>.</p></font><p><font face='宋体' size='2'>此网页同时要求 Microsoft Internet Explorer 5.01 或更高版本。</p><p align='center'><a href='http://www.microsoft.com/windows/ie/default.htm'> 单击此处安装最新的 Internet Explorer</a>.</font><br>&nbsp;</td></tr></table></object>
								...
								<tr>
									<td valign="top"><div id="TXT_BYYEAR">
									　</td>
								</tr>
							</table>
