...
<%
    ...
    String sDat = EAFunc.NVL( request.getParameter("DAT"),usrinfo.getLogdat());
    String divide=EAFunc.NVL( request.getParameter("divide"),"");
    String type=EAFunc.NVL( request.getParameter("type"),"1");
    String bascat1ab=EAFunc.NVL( request.getParameter("bascat1ab"),"A");
    ...
    try {
      ...
      else if ( sAction.equalsIgnoreCase("start") ) 
      {
         ...
         ds=db.QuerySQL(sql);
         if(!sDat.equals(ds.getStringAt(0,0)))
         {
           out.println( "<script language='javascript'>alert('日期必须为当前天的下一天');</script>" );
         }
         else
         {
           t=SCI.MoveCurord(db,usrid,sDat,type,bascat1ab);
           if(t==0)
           {
           ...
           {
               t2=SCI.divide(db,type,sDat,bascat1ab);
               out.println( "<script language='javascript'>alert('操作成功,从当前库导入了"+t+"条记录到历史库,并且在历史库中对"+t2+"笔记录拆分了送货单，您可以点击‘导出数据’按钮导出文本文件！');</script>" );
           }
         }
      }
      else if ( sAction.equalsIgnoreCase("divide") ) 
      {
         ...
         {
            out.println( "<script language='javascript'>alert('操作成功,历史订单库拆分了"+t2+"条记录!');</script>" );
         }
      }
	...
	ds = db.QuerySQL("SELECT ID,NAME FROM V_AB");
...
　　　　    <%=sBascat1ABList%>
           ...
           <input type="submit" name="button1" size="40" value="导入手机数据并拆分单号"/>
           <input type="button" onclick="javascript:f_divide();" name="button3" size="40" value="仅拆分送货单" >
           <input type="button" onclick="javascript:window.location='genso.jsp?DAT='+form1.DAT.value+'&type='+form1.type.value+'&bascat1ab='+form1.bascat1ab.value;" name="button2" size="40" value="  刷新 " >
           <!--           <input type="button" onclick="javascript:f_export_dat();" name="button2" size="40" value="  导出数据 " > -->
                      <p align='center'>
           <input type="button" onclick="javascript:f_hisord_qz();" name="button4" size=40 value="送货单取整">在生成送货单之前</P>
           <p align='center'>

           <input type="button" onclick="javascript:f_export_hisord();" name="button4" size=40 value="批量生成送货单">不分常低温，但区分条线AB类
           <p align='center'>
