...
//out.print("sssss"+bascal);
    ...
    { 
          ...
          String bascatnote = db.GetSQL("select NVL(decode('"+itemtype+"','1',curdaylintyp1,curdaylintyp2),'A') note from corp a where a.id='"+corpid+"'");
...
//              out.print(data);
              ...
              EAXmlDS bascat12ds = db.QuerySQL("select bascat12 from v_cust where id = '"+corpid+"'");
              bascat12 = bascat12ds.getStringAt(0,"bascat12");
              ...
              if (itemlist.length > 1 ) {
                for ( int i=1;i<itemlist.length;i++ ) {
                  ...
                  sItemid = vl[0];
                  soqty = vl[2];
                  EAXmlDS itemds = db.QuerySQL("SELECT GUID,UNTNUM,NAME FROM ITEM WHERE ID='"+sItemid+"'");
                  ...
                  sItemnam = itemds.getStringAt(0,"NAME");
                  for ( int j = 0;j<sctypds.getRowCount();j++ ) {
                    ...
                    plansql = "select nvl(sum(qty),99999) planqty from soplan"
                              +" where dat = to_date(to_char(sysdate,'YYYY-MM-DD'),'YYYY-MM-DD')"
                              +" and deptid = (select bascat12 from v_cust where id = '"+corpid+"')"
                              +" and brand = 'MN'"
                              +" and item = '"+sItemGUID+"'";
                    cursql = "select nvl(sum(qty),0) curqty from curord"  
                             +" where corp in(select guid from v_cust where bascat12 = (select bascat12 from v_cust where id ='"+corpid+"'))"
                             +" and item ='"+sItemGUID+"'"
                             +" and sctype ='1'";
                    planqty = db.GetSQL(plansql);
                    curqty = db.GetSQL(cursql);
//                    out.println("soqty:"+soqty);
//                     out.println("corpid:"+corpid);
//                    out.println("planqty: "+planqty+" curqty:"+curqty);
//                    num = -1;
                    if(EAFunc.isEmptyStr(soqty)){
                      soqty = "0";
                    }
                    num = Integer.parseInt(curqty)+Integer.parseInt(soqty)-Integer.parseInt(planqty);
                    if(num>0){
                      if(sScid.equalsIgnoreCase("1")){
                        outerror += " "+sItemnam+" 超出了"+bascat12+"中心的限量："+num+"个，请重新录入或咨询管理员。";
                        continue;
                        }
                    }else{
...
//                      if ( nnn == 0 ) out.println( "update curord set qty="+sqlstr+" where guid='"+sGuid+"'" );
             ...
             db.Commit();          
             out.println( "操作成功,本网点到目前共录入了"+cnt+"个商品,"+"提示："+outerror );  
