...
<%
    ...
    // out.print("locid"+locid);
     ...
     try{
         ...
         20090608修改为+10
         */          
            sql3 = "select 日期 from 日历 where to_char(日期,'YYYY-MM-DD') = to_char(sysdate+10,'YYYY-MM-DD')";
          /*
          sql4处理保鲜奶的报计划时间
          当天报后3天的计划，如1号报4号的计划
          20090519修改为+4
          */     
            sql4 = "select 日期 from 日历 where to_char(日期,'YYYY-MM-DD') = to_char(sysdate+4,'YYYY-MM-DD')";
  ...
  %>
<form id="form1" name="form1" method="post" action="j2meplanenter.jsp?brand=<%=brand%>&itemtype=<%=itemtype%>&locid=<%=locid%>&mobtyp=<%=mobtyp%>&bascat=<%=bascat%>&number=<%=number%>">
...
<p align="center">
  ...
  <%
          ...
          if(mobtyp.equalsIgnoreCase("21")){//保鲜奶
             for(int i= 0;i<c3;i++){
               dat = (String)ds3.getStringAt(i,"日期");
               sOut += "<img src=\"resource://pages/"+(i%9+1)+".png\">　<a href=\"j2meplanenter.jsp?dat="+dat+"&brand="+brand+"&itemtype="+itemtype+"&locid="+locid+"&mobtyp="+mobtyp+"&bascat="+bascat+"&number="+number+"\">"+dat+"</a><br>";
         }
      }
    }
