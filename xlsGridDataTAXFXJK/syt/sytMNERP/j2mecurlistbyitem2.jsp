...
<%       
  ...
  try
  {
        sql = "select corp.id as corpid,corp.name as corpname,ka.EXTCAT2 as flag"
               +" From usrcorp,usr,corp,v_ka ka "
               +" where usr.guid = usrcorp.usr"
               +" and usr.id = '"+sUID+"'"
               +" and usrcorp.corp = corp.guid"
               +" and ka.guid=corp.refguid"
               +" and corp.useflg='1'"
               +" order by flag,ka.id,corp.id";
    ...
    if(c>0){
     if(pageno==nPageCount)
      out.print("<a href=\"j2menewcorplist.jsp?showpage="+(pageno-1)+"\">上页</a>");
    if(pageno<nPageCount)
      out.print("<a href=\"j2menewcorplist.jsp?showpage="+(pageno+1)+"\">下页</a>");
    out.print("<br/>");
    for( int i=0;i<c;i++) 
    { 
      extcat2 = ds.getStringAt(i,"flag");
      if(extcat2.equalsIgnoreCase("1")||extcat2.equalsIgnoreCase("2")||extcat2.equalsIgnoreCase("4")||extcat2.equalsIgnoreCase("9")){
     out.print( "<img src=\"resource://pages/"+(i%9+1)+".png\">　<a href=\"j2meitemtype.jsp?corpid="+ds.getStringAt(i,"corpid")+"&showpage="+pageno+"\">"+ds.getStringAt(i,"corpname")+"</a><br>") ;
    } else if(extcat2.equalsIgnoreCase("3")){
      out.print("<img src=\"resource://pages/"+(i%9+1)+".png\"> <a href=\"j2mebasinputbil.jsp?corpid="+ds.getStringAt(i,"corpid")+"&showpage="+pageno+"\">"+ds.getStringAt(i,"corpname")+"</a><br>");
    }
   }
   if(pageno>1)
      out.print("<a href=\"j2menewcorplist.jsp?showpage="+(pageno-1)+"\">上页</a>");
