    ...
    // out.print("locid"+locid);
     ...
     try{
         ...
         sql3处理优益C的报计划时间
          ...
          4月16日报23，24，25
         */          
            sql3 = "select 日期 from 日历 "
                  +"where to_char(日期,'YYYY-MM-DD')>=to_char((select 7+decode(星期,0,sysdate,1,sysdate-1,2,sysdate-2,3,sysdate-3,"
                  +"4,sysdate,5,sysdate-1,sysdate-2 ) "
                  +"from 日历 a where to_char(日期,'YYYYMMDD')=to_char(sysdate,'YYYYMMDD')) ,'YYYY-MM-DD') "
                  +"and to_char(日期,'YYYY-MM-DD')<=to_char((select 7+decode(星期,0,sysdate+3,1,sysdate+2,2,sysdate+1,3,sysdate,"
                  +"4,sysdate+2,5,sysdate+1,6,sysdate ) "
                  +"from 日历 a where to_char(日期,'YYYYMMDD')=to_char(sysdate,'YYYYMMDD')) ,'YYYY-MM-DD') "
                  +"order by 日期 ";
          /*
          sql4处理保鲜奶每天报后天的计划
          */     
            sql4 = "select 日期+2 from 日历 where to_char(日期,'YYYY-MM-DD')=to_char(sysdate,'YYYY-MM-DD')";
