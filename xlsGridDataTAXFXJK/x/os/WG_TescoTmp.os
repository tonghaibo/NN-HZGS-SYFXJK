function x_WG_TescoTmp(){var webget = new JavaPackage("com.xlsgrid.net.webget");
...
function start()
{
	return getOrderStr("GMHD","2013-01-24","00071","T101187","b60299101187","dept","utf-8");
}
...
function getOrderStr(acc,dat,kaid,userid,passwd,deptid,code)
{
	...
	try
	{
		...
		//供应商编号
		param = "downFlag=&readFlag=&page.jumpNumber=1&page.pageNo=1&page.totalPages=2&page.pageSize=100&orderDateStart="+dat1+"&orderDateEnd="+dat1+"&parentVendor=101187&vendor=10000704&page.togglestatus=null&status=sell" ;
...
function  parseOneBill(httpcall,str,userid,deptid,kaid)
{
	...
	*/
	...
	for(var r = 1;r < title_params.length();r ++)
	{
		...
		if (corpnam.indexOf("EXPRESS") != -1) break;
