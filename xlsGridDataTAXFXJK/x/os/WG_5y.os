function x_WG_5y(){var webget = new JavaPackage("com.xlsgrid.net.webget");
...
function start()
{
	return getOrderStr("JQPX","2010-08-30","0033","JQ10","70018210","5",""); 
}
...
{ 
	...
	try{
		...
		while( pageNo < totalPage )
		{
			...
			links = getOrderList("http://61.129.104.202:82/b2b/numerous/order/listOrderAction.do","startIndex="+select_page+"&pageSize="+pageSize+"&select_page="+select_page,"GBK",dat);
			pageNo++;
		}
		//取订单明细
		...
		for (var e = 0;e < links.size();e++)
		{
			...
			for (var i = 0;i<ordlist.getRowCount();i++)
			{
				...
				if (itmid != "合计")
				{
					...
					{
						var key = header[j];
						if (key.indexOf("订货单号") > -1) 
						{
							order.setValueAt(row,"ORDID",key.substring(key.indexOf("订货单号")+5,key.length()));
							order.setValueAt(row,"BILID",key.substring(key.indexOf("订货单号")+5,key.length()));
						}	
						if (key.indexOf("订货日期") > -1) 
							order.setValueAt(row,"DAT",key.substring(key.indexOf("订货日期")+5,key.length()));
						if (key.indexOf("截止送货日期") > -1)
							 order.setValueAt(row,"ARRDAT",key.substring(key.indexOf("截止送货日期")+7,key.length()));	
					}
					order.setValueAt(row,"ECORPID","伍缘");
					order.setValueAt(row,"ECORPNAM","伍缘");
			...
			for (var i = 0;i<TR.length();i++)
			{
				...
				for (var j = 0;j<TD.length();j++)
				{
					if (j == 4)
					{
						...
						{
							links.add("http://61.129.104.202:82/b2b/numerous/order/listOrderDtlAction.do?proc=searchFixedDtl&orderNo="+TD[0].toPlainTextString().trim());
						}
					}
				}
			}
