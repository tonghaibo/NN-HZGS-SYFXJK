function x_LAYIMGBATCH(){var pubpack = new JavaPackage ( "com.xlsgrid.net.pub" );
		...
		ret+="<body>\n";
	        ...
	        if(ds.getRowCount()>0){
	        	for(var r=0;r<ds.getRowCount();r++){
	        		ret+="<a href='#' onclick=\"frames[0].location='iconbatch.sp?path=xlsgrid/images/flash/icon/&issub=2&rettyp="+ds.getStringAt(0,"typ")+"';\">"+ds.getStringAt(0,"typ")+"</a>&nbsp;";
	        	}
	        }
	       ret+= "</td></tr>"
	        ret+="<tr><td colspan=2><iframe width=100% height=100% src=\"iconbatch.sp?path="+path+"&rettyp="+rettyp+"&issub=1\" frameborder=0></td></table></body></html>";
	            ...
	            for ( var i=0;i<fileds.getRowCount();i++ ) {
	            	...
	            	if(i%8==0)
	            		ret+="<tr>";
	            	ret +="<td><table><tr><td align=center onmouseover=\"this.style.backgroundColor='#DBEBFA';this.style.cursor='hand';\" onmouseout=\"this.style.backgroundColor='#EFF7FF';\"  onclick=\"javascript:window.returnValue='"+fileds.getStringAt(i,rettyp)+"';window.close();\"><img src=\""+fileds.getStringAt(i,"URL")+"\" border=0 width=120 height=120 /><BR>"+fileds.getStringAt(i,"ID")+"</td></tr></table></td>";
	        ...
	        ret+="</tr></table>";
		     ...
		     //图片库（皮肤）
		     var typ
		     var fileds = getfilelist2 (db,rettyp);
            	     ret +="<table>";
	             for ( var i=0;i<fileds.getRowCount();i++ ) {
	            	if(i%8==0&&i!=0) 
	            		ret+="</tr>";
