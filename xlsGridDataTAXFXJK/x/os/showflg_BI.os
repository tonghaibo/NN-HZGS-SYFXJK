...
function Biwap(){
	...
	var iBI_TYP1 = request.getParameter("BI_TYP1");//显示初始图形风格
...
//	throw new Exception(sql);	
	if(ds.getRowCount()==0) return "没有查询到数据";
...
//	var vdim=db.GetSQL("select vdim from DIM_TOPIC where refmod='"+BI_GUID+"' and id='"+BI_TITID+"'");
	...
	for(var r=0; r<=rownum ; r++){
		...
		for(var c=0;c< ds.getColumnCount();c++){
			//最后一行合计
			if(r==ds.getRowCount() ){
				//throw new Exception("最后一行合计");
				if(c==0&&ds.getColumnCount()>1)
					html+="<td style=\"border:solid "+TB_BORDERCOLOR+";font-size:12px; border-width:1px "+tdborder+"px 1px 1px;\" align=center>合计</td>";
				else{
				...
				if(c==0&&ds.getColumnCount()>1)// 每行第一个单元格
				...
				if(c==ds.getColumnCount()-1&&IFROWSUM=="1"){//合计
...
function GetColname(db,colid,modguid,tablename)
{
	...
	if ( ds.getRowCount() > 0 ) ret = ds.getStringAt(0,"NAME");
		...
		ds = db.QuerySQL("select nvl(supername,'')||' '||name name from dim_target where refmod='"+modguid+"' and upper(id)=upper('"+colid+"')"); //目标
		if ( ds.getRowCount() > 0 ) ret = ds.getStringAt(0,"NAME");
		else {
