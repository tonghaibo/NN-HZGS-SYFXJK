function x_framechart(){var pub = new JavaPackage("com.xlsgrid.net.pub");
...
function Response()
{
	...
	var rightwndright = pub.EAFunc.NVL(request.getParameter("rightright"),"400");// 右边区域的宽度
	     ...
	     ret="	<html>
			...
			<script>
				...
				function switchRBar()
				{
					...
					else
					{
						...
						rightTd.style.display=''; 
   							rightTd.style.width = "+rightwndright +";
					}
				}
			    ...
			    function f_chgchart()
			    {
			    	...
			    	var ss = swf.split('-');
			    	var chart1 = new FusionCharts('xlsgrid/images/flash/'+ss[0]+'.swf', 'ChartId1', '"+rightwndright +"', '300'); 
			    	chartdatatype  = ss[1];
			    	if ( ss[1]=='1') 
		...
		<body  topmargin='0' leftmargin='0' scroll=no bgcolor='#F6F6F6' >
			...
			<tr><td >
        			<IFRAME name='_right' id='_right' frameBorder=0 width='100%' height='100%' border='0'
        				scrolling='yes' style='border: 0px solid #808080' src='"+lefturl +"' >
        			</IFRAME>	
        			</td>
        			...
        			<td id=rightTd width="+rightwndright+">
        				<table width=100% height=100% border='0' cellspacing='0' cellpadding='0' >
				...
				<tr><td valign=top>
				<IFRAME name='_grid' id='_grid' frameBorder=0 width='100%' height='100%' border='0' src='"+righturl+"'
