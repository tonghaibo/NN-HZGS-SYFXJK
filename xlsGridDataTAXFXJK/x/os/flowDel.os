function x_flowDel(){var pub= new JavaPackage("com.xlsgrid.net.pub");
...
function delDataflwGraph()
{
  var path = pub.EAOption.dynaDataRoot+ pub.EAOption.get("xmldb.file.grddb")+"/syt" + selsytid + "/" + filename;
  throw new pubpack.EAException(path);
  var f = new java.io.File(path);
  var ok = f.delete();
  if(ok)
    return "删除成功！" ;
  else
    return "删除失败，可能文件正在使用。"+ok;
}
// 得到某一个工作流process
			...
			for(var i=0;i<c;i++) {
				...
				if(!f.isDirectory() ) {
					var filename = f.getName();
	            			var index = filename.indexOf(".flow");	
					if ( index >=0  ) {
